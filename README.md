...
```bash
php artisan vendor:publish --provider="Package\Skeleton\SkeletonServiceProvider" --tag="migrations"
php artisan migrate
```

You can publish the config file with:
```bash
php artisan vendor:publish --provider="Package\Skeleton\SkeletonServiceProvider" --tag="config"
```
